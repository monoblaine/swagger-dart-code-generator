...
class SwaggerAdditionsGenerator extends SwaggerGeneratorBase {
  ...
  String generateConverterMappings(bool hasModels) {
    return '''
import 'package:chopper/chopper.dart';${options.swaggerFileExtraImports.map((i) => "\nimport '$i';").join()}

final Map<Type, Object Function(Map<String, dynamic>)> $mappingVariableName = {
  Map<String, String>: (Map<String, dynamic> json) => <String, String>{}
    ..addEntries(json.keys.map((key) => MapEntry(key, json[key] as String)))
};

const List<Interceptor> defaultInterceptors = [
  HeadersInterceptor({
  ...
  }),${options.defaultInterceptors.map((cls) => '\n$cls(),').join()}
];
''';
  }
